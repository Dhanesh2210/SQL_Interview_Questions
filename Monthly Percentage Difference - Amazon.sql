...
The output should include the year-month date and percentage change, 
...
SELECT ym,
		ROUND((total_value - LAG(total_value) OVER(ORDER BY ym)) * 100.0 / 
			   LAG(total_value) OVER(ORDER BY ym),2) AS pct
FROM cte;
